...
# Глобальный флаг для контроля состояния мониторинга
is_monitoring = False

# Путь к файлу, где будет храниться TELEGRAM_CHAT_ID
CHAT_ID_FILE = "telegram_chat_id.txt"

def save_telegram_chat_id(chat_id):
    """
    Сохраняет TELEGRAM_CHAT_ID в файл.
    :param chat_id: Строка с TELEGRAM_CHAT_ID.
    """
...
def load_telegram_chat_id():
    """
    Загружает TELEGRAM_CHAT_ID из файла. Если файл отсутствует или пуст, вызывает окно ввода.
    :return: Строка с TELEGRAM_CHAT_ID.
    """
    ...
    # Если файл пуст или отсутствует, запрашиваем ввод через окно
    while True:
        root = tk.Tk()
        root.withdraw()  # Скрыть основное окно Tkinter

        chat_id = simpledialog.askstring("Введите Telegram Chat ID",
                                         "Файл с Telegram Chat ID отсутствует или пуст. Пожалуйста, введите Chat ID:")

        if chat_id:  # Если пользователь ввел chat_id
            save_telegram_chat_id(chat_id)
            return chat_id
        else:
            # Показываем предупреждение и варианты действий
            retry = messagebox.askyesno("Ошибка",
                                        "Telegram Chat ID не может быть пустым. Хотите ввести его снова?\n"
                                        "Нажмите 'Да', чтобы повторить ввод, или 'Нет', чтобы завершить программу.")
            if not retry:  # Если пользователь выбрал завершение
    ...
    if packet.haslayer(DNS) and packet[DNS].qr == 1:  # Проверяем, является ли это DNS-ответом
        ...
        if packet[DNS].an:  # Если есть ответ (Answer Section)
            ...
            for ip in response_ips:
                ...
                if result is None:
                    continue  # Пропускаем, если IP в белом списке
                is_safe, details = result
...
SYN_threshold = 100  # Количество SYN-пакетов, после которого фиксируется атака
...
UDP_FLOOD_THRESHOLD = 100  # Минимальное количество UDP-пакетов за интервал
UDP_TIME_WINDOW = 5  # Интервал анализа в секундах
UDP_RESET_INTERVAL = 60  # Время для сброса записей
...
def detect_udp_flood(packet, current_time, send_telegram_alert):
    ...
    if packet.haslayer(UDP) and (packet.haslayer(IP) or packet.haslayer(IPv6)):
        ...
        if entry['packet_count'] >= UDP_FLOOD_THRESHOLD and current_time - entry['last_time'] <= UDP_TIME_WINDOW:
    ...
    for key in list(udp_traffic):
        if current_time - udp_traffic[key]['last_time'] > UDP_RESET_INTERVAL:
            del udp_traffic[key]
...
HTTP_FLOOD_THRESHOLD = 100  # Минимум 100 пакетов
HTTP_TIME_WINDOW = 10  # Время в секундах для анализа
HTTP_RESET_INTERVAL = 60  # Сброс через 60 секунд
...
def detect_http_flood(packet, current_time, send_telegram_alert):
    ...
    if packet.haslayer(TCP) and packet.haslayer(IP):
        ...
        if dport in [80, 443] and tcp_flags == "PA":
            ...
            if (
                ...
                and current_time - entry['last_time'] <= HTTP_TIME_WINDOW
            ):
                unique_sources = len(entry['unique_sources'])
                alert_message = (
    ...
    for key in list(http_traffic):
        if current_time - http_traffic[key]['last_time'] > HTTP_RESET_INTERVAL:
...
class NetworkMonitorApp(tk.Tk):
    def __init__(self):
        ...
        self.title("Сетевой мониторинг")
        self.geometry("800x600")

        # Вывод интерфейсов
        self.interface_label = tk.Label(self, text="Доступные интерфейсы:")
        self.interface_label.pack(pady=5)

        self.interfaces_output = scrolledtext.ScrolledText(self, height=3, wrap=tk.WORD)
        self.interfaces_output.pack(fill=tk.BOTH, expand=False, padx=10, pady=5)

        # Кнопка для начала мониторинга
        self.start_button = tk.Button(self, text="Начать мониторинг", command=self.start_sniffing)
        self.start_button.pack(pady=10)

        # Кнопка для остановки мониторинга
        self.stop_button = tk.Button(self, text="Остановить мониторинг", command=self.stop_sniffing)
        self.stop_button.pack(pady=10)

        # Поле для вывода логов
        self.logs_output = scrolledtext.ScrolledText(self, wrap=tk.WORD)
        self.logs_output.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # Настройка цветов текста
        self.logs_output.tag_config("alert", foreground="red")  # Красный текст для предупреждений
        self.logs_output.tag_config("normal", foreground="white")  # Обычный текст черным цветом

        # Отобразить доступные интерфейсы
    ...
    def display_interfaces(self):
        ...
        self.interfaces_output.insert(tk.END, " ".join(interfaces) + " ")

    def start_sniffing(self):
        """Начать мониторинг сетевого трафика в отдельном потоке."""
        self.sniffing = True
        self.logs_output.insert(tk.END, "Начинаем мониторинг сетевого трафика...\n", "normal")
    ...
    def stop_sniffing(self):
        """Остановить мониторинг сетевого трафика."""
        self.sniffing = False
        self.logs_output.insert(tk.END, "Мониторинг сетевого трафика остановлен.\n", "normal")
        self.logs_output.see(tk.END)
        self.start_button.config(state=tk.NORMAL)
        self.stop_button.config(state=tk.DISABLED)

    def sniff_traffic(self):
        """Функция для запуска sniff в отдельном потоке."""
        sniff(prn=self.process_packet, store=False, stop_filter=lambda x: not self.sniffing)

    def process_packet(self, packet):
        """Обработать пакет и вывести его информацию в логи."""
        log_message = packet_handler(packet)
        if log_message:
            # Проверяем, является ли сообщение предупреждением об атаке
            tag = "alert" if "[ALERT]" in log_message else "normal"
            self.logs_output.insert(tk.END, log_message + "\n", tag)
            self.logs_output.see(tk.END)
